/* ═══════════════════════════════════════════════════════════
   TypeCraft — Frontend JS
   Modules: TypingEngine, Dashboard, UI helpers
═══════════════════════════════════════════════════════════ */
...
/* ─────────────────────────────────────────────────────────
   DASHBOARD — Language tab switcher
───────────────────────────────────────────────────────── */
...
class TypingEngine {
  constructor(opts) {
    ...
    this.typed     = '';
    this.errors    = 0;
    this.errorSet  = new Set();   // indices of mistakes
    this.started   = false;
    this.finished  = false;
    this.startTime = null;
    this.elapsed   = 0;
  ...
  /* ── Render text as individual <span>s ────────────────── */
  _render() {
  ...
  _handleKey(e) {
    ...
    if (actual === expected) {
      ...
      if (!this.errorSet.has(idx)) {
        this.errorSet.add(idx);
        this.errors++;
      }
      // Shake on error
    ...
    // Advance cursor
    if (idx + 1 < this.chars.length) {
      this._setCurrent(idx + 1);
    } else {
      this._clearCurrent();
    }
  ...
  _updateChar(i, cls) {
    if (this.chars[i]) {
      this.chars[i].className = `char ${cls}`;
    }
  }
...
function initKeyboardHints() {
  const hints = $$('.key-hint');
  if (!hints.length) return;

  document.addEventListener('keydown', e => {
    hints.forEach(k => {
      if (k.dataset.key?.toLowerCase() === e.key.toLowerCase()) {
